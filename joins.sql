...
SELECT * FROM posts WHERE postuserid = 100;
...
SELECT posts.*, users.first_name, users.last_name FROM posts INNER JOIN users ON posts.postsusersid = users.id WHERE users.id = 200;
...
SELECT posts.*, users.username FROM posts INNER JOIN users ON posts.postsusersid = users.id WHERE users.first_name = 'Norene' AND users.last_name = 'Schmitt';


--Create a query to get usernames from the users table where the user has created a post after January 1, 2015
SELECT username FROM users LEFT JOIN posts ON posts.postsusersid = users.id WHERE posts.created_at > '2015-01-01';
...
SELECT posts.title, posts.content, users.username FROM posts LEFT JOIN users ON posts.postsusersid = users.id WHERE users.created_at < '2015-01-01';
...
SELECT comments.*, posts.title FROM comments LEFT JOIN posts ON comments.commentspostsid = posts.id;
...
SELECT comments.*, posts.title, posts.url, comments.body FROM comments LEFT JOIN posts ON comments.commentspostsid = posts.id WHERE posts.created_at < '2015-01-01';
...
SELECT comments.*, posts.title, posts.url, comments.body FROM comments LEFT JOIN posts ON comments.commentspostsid = posts.id WHERE comments.body LIKE '%USB%';
